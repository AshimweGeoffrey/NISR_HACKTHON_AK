...
import { TrendingUp, TrendingDown, Users, Activity, MapPin, DollarSign } from 'lucide-react';
...
export default function Analysis({ predictions }: AnalysisProps) {
  ...
  const waterSanitationData = [
    {
      ...
      count: predictions.filter((p) => p.input?.waterAccess === 1 && p.input?.sanitationAccess === 1).length,
    },
    {
      category: 'Water Only',
      count: predictions.filter((p) => p.input?.waterAccess === 1 && p.input?.sanitationAccess === 0).length,
    },
    {
      category: 'Sanitation Only',
      count: predictions.filter((p) => p.input?.waterAccess === 0 && p.input?.sanitationAccess === 1).length,
    },
    {
      category: 'Neither',
      count: predictions.filter((p) => p.input?.waterAccess === 0 && p.input?.sanitationAccess === 0).length,
    },
  ...
  const ageDistributionData = predictions
    .reduce((acc, pred) => {
      const ageGroup =
        pred.childAge < 6 ? '0-5 months' :
        pred.childAge < 12 ? '6-11 months' :
        pred.childAge < 24 ? '12-23 months' :
        pred.childAge < 36 ? '24-35 months' : '36+ months';
    ...
    .map((item) => ({ ...item, avgProbability: item.avgProbability / item.count }));
  ...
  const riskTrend = predictions.length >= 2
    ? predictions[predictions.length - 1].probability > predictions[0].probability ? 'up' : 'down'
    : 'neutral';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <div>
            ...
            <h3 className="text-3xl font-bold">{totalCases.toLocaleString()}</h3>
          </div>
        </motion.div>
        ...
        >
          ...
          <div>
            <p className="text-red-100 text-sm font-medium mb-1">High Risk Cases</p>
            <h3 className="text-3xl font-bold">{highRiskCases.toLocaleString()}</h3>
            <p className="text-red-100 text-xs mt-1">
              {totalCases > 0 ? ((highRiskCases / totalCases) * 100).toFixed(1) : 0}% of total
            </p>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className="p-3 bg-white bg-opacity-20 rounded-lg">
              {riskTrend === 'up' ? <TrendingUp className="w-6 h-6" /> : <TrendingDown className="w-6 h-6" />}
            </div>
          </div>
          <div>
            <p className="text-purple-100 text-sm font-medium mb-1">Average Risk</p>
        ...
        >
          ...
          <div>
            <p className="text-green-100 text-sm font-medium mb-1">Regions Affected</p>
            <h3 className="text-3xl font-bold">{regionsAffected}</h3>
            <p className="text-green-100 text-xs mt-1">Geographic coverage</p>
          </div>
        </motion.div>
      </div>
      ...
      {!hasEnoughData && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-semibold text-yellow-800">Limited Data Available</h3>
              <p className="text-sm text-yellow-700 mt-1">
                Create at least 3 predictions to unlock detailed visualizations and trends.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {riskCategoryData.length > 0 && (
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">Overview of risk levels across all predictions</p>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <p className="text-xs text-gray-500">Total Cases: {predictions.length}</p>
            </div>
          </motion.div>
        )}
        ...
        {timelineData.length > 0 && (
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">Temporal trend of prediction activity</p>
            <ResponsiveContainer width="100%" height={300}>
        ...
        {regionData.length > 0 && (
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">Geographic distribution of malnutrition risk</p>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={regionData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                <XAxis dataKey="name" stroke="#6B7280" style={{ fontSize: '12px' }} />
                <YAxis stroke="#6B7280" style={{ fontSize: '12px' }} label={{ value: 'Cases', angle: -90, position: 'insideLeft' }} />
                <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #E5E7EB', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }} />
                <Legend />
                <Bar dataKey="High" fill="#EF4444" name="High Risk" radius={[8, 8, 0, 0]} />
                <Bar dataKey="Medium" fill="#F59E0B" name="Medium Risk" radius={[8, 8, 0, 0]} />
                <Bar dataKey="Low" fill="#10B981" name="Low Risk" radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
            <div className="mt-4 pt-4 border-t border-gray-200">
              <p className="text-xs text-gray-500">Regions analyzed: {regionData.length}</p>
            </div>
          </motion.div>
        )}
        ...
        {educationData.length > 0 && (
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">Correlation between maternal education and child health risk</p>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={educationData} layout="vertical">
                ...
                <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #E5E7EB', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }} />
                <Legend />
                <Bar dataKey="High" stackId="a" fill="#EF4444" name="High Risk" radius={[0, 8, 8, 0]} />
        ...
        {incomeRiskData.length >= 2 && (
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">How household income affects malnutrition risk</p>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={incomeRiskData}>
                ...
                <XAxis
                  ...
                  stroke="#6B7280"
                  style={{ fontSize: '12px' }}
                  label={{ value: 'Household Income', position: 'insideBottom', offset: -5 }}
                />
                <YAxis stroke="#6B7280" style={{ fontSize: '12px' }} label={{ value: 'Risk %', angle: -90, position: 'insideLeft' }} />
                <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #E5E7EB' }} />
                <Legend />
                <Line
                  type="monotone"
        ...
        {foodInsecurityData.length > 0 && (
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">Relationship between food insecurity levels and health risk</p>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={foodInsecurityData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                <XAxis dataKey="level" stroke="#6B7280" style={{ fontSize: '12px' }} label={{ value: 'Insecurity Level', position: 'insideBottom', offset: -5 }} />
                <YAxis stroke="#6B7280" style={{ fontSize: '12px' }} />
                <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #E5E7EB', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }} />
                <Legend />
                <Bar dataKey="count" fill="#10B981" name="Number of Cases" radius={[8, 8, 0, 0]} />
                <Bar dataKey="avgRisk" fill="#14B8A6" name="Avg Risk %" radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </motion.div>
        )}
        ...
        {waterSanitationData.length > 0 && (
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">Infrastructure access distribution among cases</p>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
        ...
        {householdSizeData.length > 0 && (
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">How family size correlates with malnutrition risk</p>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={householdSizeData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
                <XAxis dataKey="size" stroke="#6B7280" style={{ fontSize: '12px' }} label={{ value: 'Household Size', position: 'insideBottom', offset: -5 }} />
                <YAxis stroke="#6B7280" style={{ fontSize: '12px' }} label={{ value: 'Cases', angle: -90, position: 'insideLeft' }} />
                <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #E5E7EB', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }} />
                <Legend />
                <Bar dataKey="High" stackId="a" fill="#EF4444" name="High Risk" radius={[8, 8, 0, 0]} />
                <Bar dataKey="Medium" stackId="a" fill="#F59E0B" name="Medium Risk" />
                <Bar dataKey="Low" stackId="a" fill="#10B981" name="Low Risk" />
              </BarChart>
            </ResponsiveContainer>
        ...
        {ageDistributionData.length > 0 && (
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">Child age groups and their associated risk levels</p>
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={ageDistributionData}>
                ...
                <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #E5E7EB', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }} />
                <Legend />
                <Area type="monotone" dataKey="count" stroke="#10B981" strokeWidth={2} fillOpacity={1} fill="url(#colorCount)" name="Number of Cases" />
                <Area type="monotone" dataKey="avgProbability" stroke="#14B8A6" strokeWidth={2} fillOpacity={1} fill="url(#colorRisk)" name="Average Risk %" />
              </AreaChart>
            </ResponsiveContainer>
            <div className="mt-4 pt-4 border-t border-gray-200">
              <p className="text-xs text-gray-500">Age groups analyzed: {ageDistributionData.length}</p>
            </div>
          </motion.div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <motion.div
          ...
          className="bg-white rounded-xl shadow-md p-6"
        >
          <p className="text-sm text-gray-600 font-medium mb-2 uppercase tracking-wide">Total Predictions</p>
          <div className="flex items-center justify-between">
            <h3 className="text-4xl font-bold text-gray-900">{totalCases.toLocaleString()}</h3>
          </div>
          <div className="mt-4">
            <ResponsiveContainer width="100%" height={60}>
              <AreaChart data={timelineData.slice(-7)}>
                ...
                <Area type="monotone" dataKey="predictions" stroke="#10B981" strokeWidth={2} fillOpacity={1} fill="url(#miniGradient1)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-white rounded-xl shadow-md p-6"
        >
          <p className="text-sm text-gray-600 font-medium mb-2 uppercase tracking-wide">High Risk Cases</p>
          <div className="flex items-center justify-between">
            <h3 className="text-4xl font-bold text-gray-900">{highRiskCases.toLocaleString()}</h3>
          </div>
          <div className="mt-4">
            <ResponsiveContainer width="100%" height={60}>
              <AreaChart data={timelineData.slice(-7)}>
                ...
                <Area type="monotone" dataKey="predictions" stroke="#14B8A6" strokeWidth={2} fillOpacity={1} fill="url(#miniGradient2)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-white rounded-xl shadow-md p-6"
        >
          <p className="text-sm text-gray-600 font-medium mb-2 uppercase tracking-wide">Avg Risk Level</p>
          <div className="flex items-center justify-between">
        ...
        <motion.div
          ...
          className="bg-white rounded-xl shadow-md p-6"
        >
          <p className="text-sm text-gray-600 font-medium mb-2 uppercase tracking-wide">Regions Covered</p>
          <div className="flex items-center justify-between">
            <h3 className="text-4xl font-bold text-gray-900">{regionsAffected}</h3>
          </div>
          <div className="mt-4">
            <ResponsiveContainer width="100%" height={60}>
              <BarChart data={regionData}>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <p className="text-sm text-green-600 font-medium mb-1">Most Common Risk</p>
            <p className="text-2xl font-bold text-green-800">
              {riskCategoryData.sort((a, b) => b.value - a.value)[0]?.name || 'N/A'}
            </p>
          </div>
          <div className="bg-teal-50 border border-teal-200 rounded-lg p-4">
            <p className="text-sm text-teal-600 font-medium mb-1">Average Probability</p>
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
            <p className="text-sm text-emerald-600 font-medium mb-1">Most Affected Region</p>
