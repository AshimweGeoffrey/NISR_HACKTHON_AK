...
export default function Prediction({ onPredictionComplete }: PredictionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        "https://mal-nutrition-fastapi.onrender.com/predict",
        {
          method: "POST",
      ...
      const calculateRisk = () => {
        ...
        else if (formData.household_income >= 200000)
          riskScore -= 8; // Middle income
        else if (formData.household_income >= 100000)
          riskScore += 5; // Low-middle income
        else if (formData.household_income >= 50000)
          riskScore += 12; // Low income
        ...
        else if (formData.age_months < 12)
          riskScore += 10; // 6-12 months vulnerable
        else if (formData.age_months < 24)
          riskScore += 5; // 12-24 months at risk
      ...
      const finalProbability =
        apiProbability > 0 ? apiProbability : calculateRisk();
      ...
      const finalConfidence =
        apiConfidence > 0 ? apiConfidence : calculateConfidence();
      ...
      if (
        data.risk_category &&
        ["Low", "Medium", "High"].includes(data.risk_category)
      ) {
      ...
      const generateRecommendations = () => {
        ...
        if (formData.food_insecurity > 2) {
          recommendations.push(
            "Address food insecurity through nutrition programs"
          );
        }
        ...
        if (formData.diarrhea_last_week === 1) {
          recommendations.push(
            "Seek immediate medical attention for diarrhea treatment"
          );
        }
        if (formData.stunting_risk_score > 0.5) {
          recommendations.push(
            "Monitor growth and provide nutritional supplementation"
          );
        }
        if (
          formData.mother_education === "None" ||
          formData.mother_education === "Primary"
        ) {
          recommendations.push(
            "Provide maternal health education and nutrition counseling"
          );
        }

        if (recommendations.length === 0) {
          recommendations.push(
            "Continue current health practices and regular check-ups"
          );
        }
  ...
  const handleChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: ["region", "rural_urban", "mother_education"].includes(name)
        ? value
        : parseFloat(value) || 0,
    }));
  };
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Improved Sanitation (0=No, 1=Yes)
                </label>
                <input
                  type="number"
            ...
            <motion.button
              ...
              type="submit"
              disabled={loading}
              className="w-full btn-primary py-3 rounded-lg font-medium transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {loading ? (
                <>
            ...
            >
              <div className="text-center">
                {result.riskCategory === "High" && (
                  <div className="inline-flex items-center justify-center w-20 h-20 bg-accent-red-50 rounded-full mb-4">
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-3xl font-bold text-gray-800">
                  {result.probability?.toFixed(1) || 0}%
                </p>
              </div>
              <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="bg-primary-50 border border-primary-200 rounded-lg p-4">
                ...
                <p className="text-sm text-primary-600">{result.notes}</p>
              </div>
            </motion.div>
          ) : (
